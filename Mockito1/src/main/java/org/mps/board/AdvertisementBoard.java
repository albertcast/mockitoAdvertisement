...
public class AdvertisementBoard {
    ...
    public void publish(Advertisement advertisement,
        ...
        Boolean found = false;
        List<Advertisement> list = advertisementList.stream().filter(ad -> ad.title.equals(advertisement.title)).toList();
        Iterator<Advertisement> iterator = list.iterator();
        while(iterator.hasNext() && !found){
            Advertisement aux = iterator.next();
            if(aux.title.equalsIgnoreCase(advertisement.title) && aux.advertiser.equalsIgnoreCase(advertisement.advertiser)){
                found = true;
            }
        }
